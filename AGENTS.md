...
- `dist/llms.txt` is the machine reader's guide to the site: counts, URL patterns, every winner list named explicitly
  (per prize, per category, per subject), and the two pages that carry embedded JSON. It is generated by
  `write_llms_txt` from the same plan the pages come from, so its numbers, titles, and links stay true without hand
  editing. It is not a route and stays out of the sitemap.
