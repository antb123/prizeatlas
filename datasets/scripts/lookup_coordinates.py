...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("query", help="name, English Wikipedia title, or Wikidata QID")
    ...
    try:
        qid, entity, coordinate, match_method = resolve(query)
    except LookupFailure as error:
        print(f"lookup failed: {error}", file=sys.stderr)
        return 1

    json.dump(geojson_feature(query, qid, entity, coordinate, match_method), sys.stdout, ensure_ascii=False, indent=2, allow_nan=False)
    print()
    return 0
