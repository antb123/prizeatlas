...
2. **Load-bearing:** “No editorial/ranking framing” excludes visible rank positions, scores, score bars, blurbs, the “An editorial ranking” eyebrow, and “Ranked awards” wording from the new page.
3. The new page uses a neutral `Awards` heading, then the award names, existing prize-page links, optional logos, and official-site links already present in each `Ranking`.
...
| `website/templates/awards.html` | new file | Extend `base.html`; render a neutral `Awards` heading followed only by the award-family entries from `prizes`. Reuse existing ranking-list/logo styling, internal prize routes, and official URLs, but omit rank, score, blurb, homepage totals, and homepage highlight sections. |
| `website/build.py` | 42-70, 71-96 | Register `awards.html` in `TEMPLATES` and define `AWARDS_ROUTE = "/awards/"`. |
...
Expected implementation scope: 4 files, including one new template, and approximately 40-60 changed or added implementation/test lines.
...
- The new page MUST NOT contain “Prestigious Awards and Winners”, “An editorial ranking”, “Most decorated”, “Recently awarded”, “Ranked awards”, totals, rank numbers, scores, score bars, or ranking blurbs.
...
Do not edit `website/templates/index.html:1-75`, ranking data, prize detail routes, `website/static/style.css`, `about.html`, `llms.txt`, or generated `website/dist/` output. Do not change award ordering, scores, blurbs, homepage content, the site-name home link, or any database content or schema.
